...
% hR_ratio0 = 0.025:0.025:0.25;   % The ratio h/R
% h = R0*hR_ratio;   % [mm] The thickness of the ring
h = 40;
I2 = b*h^3/12;    % The moment of intertia of the cross-section of the ring
A2 = b*h;
delta1 = 40;      % The vertical deflection of the plate
% delta1 = 1:1:50;
Error = 8e-7;
...
Cnt_ang0 = acos((R0+h/2-delta0)/(R0+h/2));
% N_ang = 1000; % The number of dividing regions (Due to the symmetry, N_ang should be even)
D_ang = 2*Cnt_ang0/N_ang;
% ang_i = [-Cnt_ang0:D_ang:-D_ang, D_ang:D_ang:Cnt_ang0]'; %The rotation angle of the force pair
ang_i = (-Cnt_ang0:D_ang:Cnt_ang0)';
...
% Road function for calculating geometry errors
for i1 = 1:length(theta_di)
    Road(i1) = (R0+h/2-delta0)/cos(theta_di(i1));
end
...
for i3 = 1:length(theta_di)
    for i4 = 1:length(D_Fr)
        Cnt_ur0(i4) = D_Fr(i4)*(R0+h/2)*D_ang*ur_bar_EA_GA_PairForce_sym(theta_di(i3),theta_di(i4),R0,E2,G,I2,A2);
        Cnt_ut0(i4) = D_Fr(i4)*(R0+h/2)*D_ang*ut_bar_EA_GA_PairForce_sym(theta_di(i3),theta_di(i4),R0,E2,G,I2,A2);
        Cnt_fi0(i4) = D_Fr(i4)*(R0+h/2)*D_ang*ufi_bar_EA_GA_PairForce_sym(theta_di(i3),theta_di(i4),R0,E2,G,I2,A2);
    end
...
% r_deform = r_deform + Cnt_ur; % The total deformation of the contact region
...
if max(abs(Cnt_ur)) < Error      %4e-7 2.5e-6
    break
else
    Cnt_ur = zeros(length(ang_i),1);
    Cnt_ut = zeros(length(ang_i),1);
...
F_i_0(:,DD) = F_i;          % The final distributed load. Contact pressure p=F_i_0/b
F_v = zeros(length(F_i),1); % Vertical reaction force
