...
A2 = b*h;
delta1 = 20;      % The vertical deflection of the plate
% delta1 = 2:2:30;
...
Y_Rc_0 = zeros(N_ang+1,length(delta1));
F_ver = zeros(length(delta1),1);

for DD = 1:length(delta1)
delta0 = delta1(DD);

%% Define the origin of the convex surface
d_c = R_c-delta0+(R0+h/2); % The distance between the road center and ring center
x_c = d_c; 
y_c = 0;          % The position of the circular center of the road

%% Calculating the force applied to the contact region

Cnt_ang0 = acos(((R0+h/2)^2+d_c^2-R_c^2)/(2*(R0+h/2)*abs(d_c))); % The initial contact region of the ring
...
ang_cp = (pi/5-pi:-pi/1800:-pi/5-pi)'; %The range of the curved plate
x_Rc0 = zeros(length(ang_cp),1);
y_Rc0 = zeros(length(ang_cp),1);

for i1 = 1:length(ang_cp)
    x_Rc0(i1) = x_c + R_c*cos(ang_cp(i1));
    y_Rc0(i1) = y_c + R_c*sin(ang_cp(i1));
end

poly_c = polyfit(y_Rc0,x_Rc0,5);
...
y_Rc = zeros(length(theta_di),1);
Road = zeros(length(theta_di),1);
theta_Road = zeros(length(Road),1);

for j0 = 1:20000 % The maximum iteration step

[x_di,y_di] = pol2cart(theta_di,r_deform);

for i_R = 1:length(y_di)
    y_Rc(i_R) = polyval(poly_c,y_di(i_R));
    Road(i_R) = sqrt(y_Rc(i_R)^2+y_di(i_R)^2);
    theta_Road(i_R) = atan(y_di(i_R)/y_Rc(i_R));
end
